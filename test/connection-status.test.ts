...
async function run(client: Z2MClient) {
  return (await tool.handler({}, { client, config: config() })) as {
...
describe("z2m_connection_status", () => {
  ...
  it("reports an unreachable broker without throwing", async () => {
    const out = await run(fakeClient(new Error("connack timeout"), { connected: false }));
